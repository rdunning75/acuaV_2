<template>
    <Page>
        <ActionBar ios.position="left" class="Action_Bar">
            <Label text="Welcome to" fontSize="20" verticalAlignment="center"/>
            <ActionItem @tap="toFaq" ios.systemIcon="12" ios.position="right" android.systemIcon="ic_menu_search"
                        android.position="actionBar"/>
        </ActionBar>
        <StackLayout backgroundColor="#b21a82">

            <GridLayout columns="*,*,*" class="layout cover">
                <ScrollView row="0" col="0" backgroundColor="#D3D3D3">
                    <StackLayout>
                        <CategoryButton
                                src="~/assets/images/animals.icon.png" :icon="''"
                                :title="titles[0]" :action="toAdoption" class="homepage-blue HomeButton"
                                textWrap="true"/>

                        <!--            Service Request -->
                        <CategoryButton
                                src="~/assets/images/services.icon.png" :icon="''"
                                :title="titles[2]" :action="toServiceRequest" class="homepage-blue HomeButton"
                                textWrap="true"/>

                        <!--            FAQ -->
                        <CategoryButton
                                src="~/assets/images/noise.icon.png" :icon="''"
                                :title="titles[3]" :action="toFaq" class="homepage-blue HomeButton"
                                textWrap="true"/>

                        <!--            Ticket -->
                        <CategoryButton
                                src="~/assets/images/hoursicon.png" :icon="'fa-map-marker' | fonticon"
                                :title="titles[4]" :action="toQRScanner"
                                class="fa homepage-blue HomeButton" textWrap="true"/>

                        <!--            Find a Location -->
                        <CategoryButton
                                src="~/assets/images/locationicon.png" :icon="''"
                                :title="titles[5]" :action="toLocationList" class="homepage-blue HomeButton"
                                textWrap="true"/>
                ...
                <GridLayout row="0" col="1" class="layout cover" columns="*,*" rows="*,*,*" colSpan="2">
                    <!--            Adoption -->
